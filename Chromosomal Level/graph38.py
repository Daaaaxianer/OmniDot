...
def tree_y_gene_1(y_gene,mod,n_2,color_date,colors,ax,date_colors,n,longs):
...
def tree_x_gene_1(x_gene,mod,m_2,color_date,colors,ax,date_colors,m,longs):
        ...
        if mod == "tpos":
            ...
            if color_date == "True":
                ...
                for i in range(len(pos.xys)):
                    ...
                    ax.text(pos.date_xy[i], name_ys[i], f'{pos.genes[i]}',
                            va='bottom', ha='center', fontsize=8, rotation=90)
def ha_f_set(ax,m,n_2,n,m_2,mod):
    ax2 = ax.twinx()  # 添加右侧y轴
...
@click.command()
@click.option('-d', '--date_path', default='config.toml', help='input you date path')
@click.option('-o', '--operation', default='blast', help='input you operation mod')
def main(date_path,operation):
    # 将所有配置变量放在一个字典中
    config_dict = {
        'mods': "blast",
        'mod': "torder",
        'name_1': "NAN", 'name_2': "NAN", 'name_3': "NAN", 'name_4': "NAN",
        ...
        'wide': 400,
        'longs': 350,
        'name_1_4': "NAN", 'name_2_4': "NAN",
        'posd': "bot_left",
        'Collinearity_ks': "NAN", 'Collinearity': "NAN", 'blast_name': "NAN",
        'blast_gff_1': "NAN", 'blast_gff_2': "NAN",
        'blast_lens_1': "NAN", 'blast_lens_2': "NAN", 'Collinearity_2': "NAN",
        'Collinearity_ks_2': "NAN",
        'blast_name_2': "NAN", 'blast_gff_1_2': "NAN", 'blast_gff_2_2': "NAN",
        'blast_lens_1_2': "NAN", 'blast_lens_2_2': "NAN", 'gene_show': "NAN",
        'savefile': "NAN"
    }

    current_vars = list(config_dict.keys())

    try:
        print(f"🔍 调试信息:")
        print(f"   date_path 值: {repr(date_path)}")
        print(f"   date_path 类型: {type(date_path)}")
        print(f"   文件是否存在: {os.path.exists(date_path)}")
        with open(date_path, "r", encoding="utf-8") as f:
            config = toml.load(f)
        print(f"🔍 调试信息:")
        print(f"   配置文件路径: {date_path}")
        print(f"   操作类型: {operation}")
        print(f"   配置文件中包含的节: {list(config.keys())}")
        updated = 0
        missing = 0
        if operation in config:
            new_config = config[operation]
            print(f"✅ 找到配置节: {operation}")
            print(f"   配置项数量: {len(new_config)}")

            # 更新配置字典
            for var_name in current_vars:
                if var_name in new_config:
                    try:
                        old_value = config_dict[var_name]
                        new_value = new_config[var_name]
                        config_dict[var_name] = new_value
                        updated += 1
                        print(f"   ✅ 更新 {var_name}: {repr(old_value)} -> {repr(new_value)}")
                    except Exception as e:
                        print(f"   ❌ 更新 {var_name} 失败: {str(e)}")
                else:
                    missing += 1
                    print(f"   ⚠ 使用默认值 {var_name}: {repr(config_dict[var_name])}")

            print(f"📊 配置更新统计: 更新了 {updated} 个参数，{missing} 个参数使用默认值")
        else:
            print(f"❌ 错误: 配置文件中未找到节 '{operation}'")
            return
    ...
    # 显式地将字典值赋给变量（更可靠的方式）
    mods = config_dict['mods']
    mod = config_dict['mod']
    name_1 = config_dict['name_1']
    name_2 = config_dict['name_2']
    name_3 = config_dict['name_3']
    name_4 = config_dict['name_4']
    collinearity_ks_mod = config_dict['collinearity_ks_mod']
    Ks = config_dict['Ks']
    ks_mod = config_dict['ks_mod']
    name_id = config_dict['name_id']
    N = config_dict['N']
    NSD = config_dict['NSD']
    Distance = config_dict['Distance']
    half = config_dict['half']
    block = config_dict['block']
    top_block = config_dict['top_block']
    ks_dem_1 = config_dict['ks_dem_1']
    ks_dem_1_1 = config_dict['ks_dem_1_1']
    ks_dem_2 = config_dict['ks_dem_2']
    ks_dem_2_2 = config_dict['ks_dem_2_2']
    pos = config_dict['pos']
    mod_show = config_dict['mod_show']
    bin_sizes = config_dict['bin_sizes']
    color_date = config_dict['color_date']
    color_dates = config_dict['color_dates']
    wide = config_dict['wide']
    longs = config_dict['longs']
    name_1_4 = config_dict['name_1_4']
    name_2_4 = config_dict['name_2_4']
    posd = config_dict['posd']
    Collinearity_ks = config_dict['Collinearity_ks']
    Collinearity = config_dict['Collinearity']
    blast_name = config_dict['blast_name']
    blast_gff_1 = config_dict['blast_gff_1']
    blast_gff_2 = config_dict['blast_gff_2']
    blast_lens_1 = config_dict['blast_lens_1']
    blast_lens_2 = config_dict['blast_lens_2']
    Collinearity_2 = config_dict['Collinearity_2']
    Collinearity_ks_2 = config_dict['Collinearity_ks_2']
    blast_name_2 = config_dict['blast_name_2']
    blast_gff_1_2 = config_dict['blast_gff_1_2']
    blast_gff_2_2 = config_dict['blast_gff_2_2']
    blast_lens_1_2 = config_dict['blast_lens_1_2']
    blast_lens_2_2 = config_dict['blast_lens_2_2']
    gene_show = config_dict['gene_show']
    savefile = config_dict['savefile']

    # 现在可以直接使用这些变量
    print(f"blast_gff_1: {blast_gff_1}")
    ...
    if mods == "blast":
        ...
        if mod_show == "nan":
            ...
            fig.canvas.draw()   #强制重新绘制图形 canvas（画布）
        if mod_show == "tree":
            fig = plt.figure(figsize=(10, 10))  # 整体图形大小调整
            # 使用gridspec创建3个区域
        ...
        if mod_show == "tree":
            ...
            x_gene=get_tree_2(x_gene,wide)
            y_gene = get_tree_2(y_gene,wide)
            colors, date_colors=tree_co_1(color_date,color_dates)
            tree_y_gene_1(y_gene, mod, n_2, color_date, colors, ax, date_colors, n,longs)
            tree_x_gene_1(x_gene, mod, m_2, color_date, colors, ax, date_colors, m,longs)
        if half == "False":
            ha_f_set(ax, m, n_2, n, m_2,mod)
